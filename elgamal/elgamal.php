...
class ElGamalASN1
{
    ...
    private static function bcdeciToBinary($dec): string {
        if ($dec === '0') return "\x00";
        $bin = '';
        while (bccomp($dec, '0') > 0) {
            $byte = bcmod($dec, '256');
            $bin = chr((int)$byte) . $bin;
            $dec = bcdiv($dec, '256', 0);
        }
        return $bin;
    }

    private static function binaryToBcDec(string $bin): string {
        $dec = '0';
        for ($i = 0; $i < strlen($bin); $i++) {
            $dec = bcmul($dec, '256');
            $dec = bcadd($dec, (string)ord($bin[$i]));
        }
        return $dec;
    }

    private static function encodeAsn1Integer(string $bin): string {
        if (ord($bin[0]) & 0x80) {
            $bin = "\x00" . $bin;
        }
        $len = strlen($bin);
        $lenEnc = $len < 128 ? chr($len) : chr(0x80 | strlen($lenBytes = ltrim(pack("N", $len), "\x00"))) . $lenBytes;
        return "\x02" . $lenEnc . $bin;
    }

    private static function encodeSequence(array $elements): string {
        $content = implode('', $elements);
        $len = strlen($content);
        $lenEnc = $len < 128 ? chr($len) : chr(0x80 | strlen($lenBytes = ltrim(pack("N", $len), "\x00"))) . $lenBytes;
        return "\x30" . $lenEnc . $content;
    }

    private static function decodeAsn1Integer(string $data, int &$offset): string {
        if (ord($data[$offset++]) !== 0x02) throw new Exception("Esperado INTEGER.");
        $len = ord($data[$offset++]);
        if ($len & 0x80) {
            $n = $len & 0x7F;
            $len = 0;
        ...
        $offset += $len;
        return self::binaryToBcDec(ltrim($val, "\x00")); // remove padding
    }

    private static function decodeSequence(string $data, int &$offset): int {
        if (ord($data[$offset++]) !== 0x30) throw new Exception("Esperado SEQUENCE.");
        $len = ord($data[$offset++]);
        if ($len & 0x80) {
            $n = $len & 0x7F;
            $len = 0;
    ...
    public static function encode_ciphertext(array $cipher): string {
        $c1 = self::encodeAsn1Integer(self::bcdeciToBinary($cipher['c1']));
        $c2 = self::encodeAsn1Integer(self::bcdeciToBinary($cipher['c2']));
        return self::encodeSequence([$c1, $c2]);
    }

    public static function encode_signature(array $sig): string {
        $r = self::encodeAsn1Integer(self::bcdeciToBinary($sig['r']));
        $s = self::encodeAsn1Integer(self::bcdeciToBinary($sig['s']));
        return self::encodeSequence([$r, $s]);
    }

    // ---- Decodificação

    public static function decode_ciphertext(string $der): array {
        $offset = 0;
        $end = self::decodeSequence($der, $offset);
        $c1 = self::decodeAsn1Integer($der, $offset);
        $c2 = self::decodeAsn1Integer($der, $offset);
        if ($offset !== $end) throw new Exception("Dados extras em ciphertext.");
        return ['c1' => $c1, 'c2' => $c2];
    }

    public static function decode_signature(string $der): array {
        $offset = 0;
        $end = self::decodeSequence($der, $offset);
        $r = self::decodeAsn1Integer($der, $offset);
        $s = self::decodeAsn1Integer($der, $offset);
