...
class AnubisEAC {
    ...
    public function encryptEAC($key, $nonce, $header, $plaintext) {
        // Setup com a chave
        $this->keySetup($key);

        // 1. Tag do header
        $tagHeader = $this->generate("\x00" . $header);

        // 2. Tag do nonce → vira o IV do modo CTR
        $tagNonce = $this->generate("\x01" . $nonce);

        // 3. Cifra em CTR
        $ciphertext = $this->ctrMode($plaintext, $key, $tagNonce);

        // 4. Tag do texto cifrado
        $tagMessage = $this->generate("\x02" . $ciphertext);

        // 5. TAG final = XOR dos 3 tags
        $finalTag = $tagHeader ^ $tagNonce ^ $tagMessage;

        return [$ciphertext, $finalTag];
    }

    public function decryptEAC($key, $nonce, $header, $ciphertext, $tag) {
        // Setup com a chave
        $this->keySetup($key);
        ...
        $tagNonce  = $this->generate("\x01" . $nonce);
        $tagMessage = $this->generate("\x02" . $ciphertext);
        $expectedTag = $tagHeader ^ $tagNonce ^ $tagMessage;

        if (!hash_equals($expectedTag, $tag)) {
            throw new Exception("Tag de autenticação inválida");
        }

        // Decifra com CTR
        return $this->ctrMode($ciphertext, $key, $tagNonce);
    }
}
